...
{ 
    hide_All_Subpages(); 
    //$("#subPage_SelectArea").removeClass("servir_helper_hidden");    need in index_old_g
...
function convert_PolygonString_To_4326_ForRequest(polygonString)
{
    ...
    if (polyObj.coordinates) {
        ...
        for (var i = 0; i < polyObj[0].coordinates[0].length; i++) {
            polyObj[0].coordinates[0][i] = ol.proj.transform(polyObj[0].coordinates[0][i], 'EPSG:102100', 'EPSG:4326');
        }
    }
...
function DisplayLayersUI() {
    if (true) {
       ...
       // gLayers[0].setVisible(true);
        ...
        gLayers[0].setVisible(false);
    }


}
//function DisplayLayersSMAP() {
   
//    if (document.getElementById('smapLayer').checked) {
//        if (!gLayers[1])
//        addLayerSMAP();
//        else
//        gLayers[1].setVisible(true);
       
//    }

//    else {
//        gLayers[1].setVisible(false);
//    }

//}
$('#contrast').on('input', function () {
    alert("hi");
    gLayers[1].setOpacity($(this).val());
});
...
function init_ToolTips()
{
    if ($("#popup_SelectData_DataSource_ToolTip1").css("display") == "none") {
        $('#helpIcon_SelectData_DataSource_ToolTip').popup({ hoverable: true, position: 'right center', delay: { show: 300, hide: 800 }, popup: $('#popup_SelectData_DataSource_ToolTip') });

    }
     if ($("#popup_SelectData_Calculations_ToolTip1").css("display") == "none") {
        $('#helpIcon_SelectData_Calculations_ToolTip').popup({ hoverable: true, position: 'right center', delay: { show: 300, hide: 800 }, popup: $('#popup_SelectData_Calculations_ToolTip') });
...
function set_Dynamic_Element_Defaults()
{
    ...
    selectData_typeOfRequest_Changed('xx');
}
// //////////////////////////////////// Page Entry Point ////////////////////////////////////
